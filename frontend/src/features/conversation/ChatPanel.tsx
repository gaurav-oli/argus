...
/**
 * Shared Ask-AI chat panel (Stories 7.1/7.2). A dismissible centered-modal chat whose answers are
 * grounded server-side; the caller supplies a {@link sendFn} that posts the thread to the relevant
 * endpoint. Stateless: the full thread is sent each turn and lives only for the open session
...
export function ChatPanel({
  ...
  placeholder: string;
  sendFn: (messages: ChatMessage[], signal?: AbortSignal) => Promise<ChatMessage>;
  onClose: () => void;
}) {
  ...
  // [onClose] effect so an unstable onClose (e.g. an inline closure from a re-rendering parent
  // like the TopBar launcher) can't abort a request still in flight.
  ...
  async function send() {
    ...
    try {
      const reply = await sendFn(next, controller.signal);
