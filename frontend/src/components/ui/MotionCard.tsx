...
export function MotionCard({
  ...
  index = 0,
  ...rest
}: React.ComponentProps<typeof motion.div> & { interactive?: boolean; index?: number }) {
  const reduce = useReducedMotion();
  return (
    <motion.div
      initial={reduce ? false : { opacity: 0, y: 16, scale: 0.985 }}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={reduce ? { duration: 0 } : { delay: index * 0.07, type: "spring", stiffness: 120, damping: 18 }}
      ...
      className={cn(
        ...
        "shadow-[0_1px_0_0_rgba(255,255,255,0.04)_inset,0_18px_40px_-20px_rgba(0,0,0,0.8)]",
        className,
      )}
      {...rest}
    >
