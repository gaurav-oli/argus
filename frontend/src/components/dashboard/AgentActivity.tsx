...
import { agentNodes } from "@/lib/mockData";

/**
 * Live view of the agent fleet: each agent streams signal particles down a wire
 * into the central Argus core. Active agents pulse + flow; idle agents are dim.
 */
      ...
      <div className="mt-4 flex items-stretch gap-3">
          ...
          {agentNodes.map((a, i) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3">
                <div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03]">
                  {on && !reduce && (
                ...
                <div className="w-24 shrink-0">
                  <p className={`text-sm font-medium ${on ? "text-text-primary" : "text-text-secondary"}`}>{a.name}</p>
                  <p className="font-mono text-[10px] text-text-secondary">{a.role}</p>
                </div>

                {/* flowing wire */}
                <div className="relative h-px flex-1 bg-gradient-to-r from-white/5 via-white/10 to-accent/30">
                  {on &&
                    !reduce &&
                    [0, 1, 2].map((k) => (
                      <motion.span
        ...
        <div className="flex items-center">
          <div className="relative flex h-20 w-20 flex-col items-center justify-center rounded-2xl border border-accent/30 bg-accent/[0.06]">
            ...
            <span className="h-3 w-3 rounded-full bg-accent" style={{ boxShadow: "0 0 16px #00D4FF" }} />
