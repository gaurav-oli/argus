...
- ~~**Model failure surfaces as HTTP 200 with the Haiku-stub text (decision deferred to 7.3).**~~ **RESOLVED by Story 7.3 (2026-06-23):** `StubHaikuFallback` was replaced by a real `AnthropicHaikuFallback` (key present) / `UnavailableHaikuFallback` (no key, throws `ModelGatewayException`), and `ModelGatewayException` now maps to **503 Problem Details** (`GlobalExceptionHandler`). A model/escalation failure is a clean 503 the frontend surfaces — no placeholder text returned as a 200. (Real-key + on-failure-fallback live check is on the Mini checklist §7.)
...
## Deferred from: story-7.2 (2026-06-23)

- **Persisted investor profile.** Portfolio chat grounds "investor profile" from a static constant (`ConversationService.INVESTOR_PROFILE` — Canadian solo investor, CAD, TFSA/RRSP) because no profile/investor entity exists. The full **Canadian Investor lens** (CAD-equivalents, TFSA/RRSP eligibility, withholding tax) is **Story 7.5 (FR-34)**, and a persisted/editable profile entity is future work — when it lands, source the chat profile from it instead of the constant.
- **Portfolio-chat shares the deferred items from 7.1:** the Haiku-stub-on-failure HIGH (→ 7.3), no client-side timeout/queued feedback on the serialized BIG gateway (→ streaming/pre-warm, Mini), and silent `MAX_TURNS` truncation — all apply equally to `/api/portfolio/chat`.
- **Calendar grounding omits the quiet-period annotation.** The chat lists upcoming events (type/ticker/title/date/daysUntil) straight from `CalendarEventRepository`; it does not call `EarningsQuietPeriodService` to annotate earnings with CLEAR/NOTE/QUIET (as the `/api/calendar/upcoming` UI does). Add if the model needs the quiet-period posture in answers.

## Deferred from: code review of story-7.2 (2026-06-23)

## Deferred from: story-7.3 (2026-06-23)

- **Auto-detection of complex queries.** 7.3 ships only user-triggered escalation ("Get deeper analysis"). Model-decided auto-escalation (FR-32 "auto-detected") is deferred — a simple heuristic risks false triggers + cost; revisit with Phase-1 monitoring of the assumed <10% escalation rate.
- **Budget governance is Epic 10.** 7.3 records per-call cost (`CostRecorder`, in-memory + structured log) only. Persisted per-call/agent/model spend + monthly reset (Story 10.5) and the 70/80/95% thresholds + notification-first auto-switch to local (Story 10.6 / FR-45/46) are the Cost Governor — not built here. When 10.5 lands, persist what `CostRecorder` logs.
- **Cost not yet shown in the UI.** Escalated answers are tagged "via Claude Haiku" in the chat, but spend is visible only in logs/in-process; the API budget panel is the Ops dashboard (Epic 9/10).
- **Escalation has no concurrency/budget gate (code review).** `ModelGateway.escalate` deliberately bypasses the big-model semaphore (Haiku is remote), and 7.3 adds no per-call budget/rate limit — N concurrent "Get deeper analysis" = N paid calls. The frontend `pending` guard bounds a single panel only. Per-call budget enforcement + the 70/80/95% gate + auto-switch are the **Cost Governor (Epic 10, Stories 10.5/10.6)**; wire escalation through it when built.
- **Sanitized Haiku context still includes tickers + company names + exact weights (code review).** The hard privacy barrier (no exact dollar amounts / share counts / portfolio total) holds, but the holdings list + relative weights could let a recipient estimate sizes via public market caps. Accepted posture for useful analysis; drop company names (or coarsen weights) if a stricter "no raw positions" bar is later required.
- **Spring AI Anthropic built manually.** Because Spring AI's chat-model selector (`spring.ai.model.chat`) is mutually exclusive, Anthropic autoconfig is disabled (`=ollama`) and the Haiku client is built by hand in `HaikuConfig` (needs the `anthropic-java-client-okhttp` dep). If Spring AI later supports multiple chat providers cleanly, simplify to autoconfig + `@Qualifier`.

- **Shared chat dialog a11y gaps (inherited from 7.1, now global).** `ChatPanel` (extracted from `RecommendationChat`) has `role="dialog"` + `aria-label` but no `aria-modal="true"`, no focus trap, no initial focus, and no body-scroll-lock — Tab can reach the page behind the backdrop and the background scrolls. Carried over unchanged from 7.1; the new TopBar launcher exposes it on every dashboard page. Fix once for the shared panel (focus-trap + `aria-modal` + scroll-lock) — benefits both recommendation and portfolio chat.
